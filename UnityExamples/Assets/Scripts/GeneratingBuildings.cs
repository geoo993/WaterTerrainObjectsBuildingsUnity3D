...
public class GeneratingBuildings : MonoBehaviour {
	...
	void CreateBuildings ()
	{
		...
		float xBound = map.GetComponent<MeshRenderer>().bounds.size.x;
		float zBoumd = map.GetComponent<MeshRenderer>().bounds.size.z;

		GameObject city = new GameObject ();
		
		for (var i = 0; i < 4000; i ++) {
			
			GameObject buildingMesh = CreateCubeOrCuboid();
			buildingMesh.transform.parent = city.transform;
			buildingMesh.name = "cuboiddObject" + i;

			float xPos = Random.Range (-xBound/2, xBound/2);
			float zPos = Random.Range (-zBoumd/2, zBoumd/2);
